...
function Room({ room, username, onLeave }) {
  ...
  const addEmoji = (emoji) => { changeMessage(`${message}${emoji}`); setEmojiOpen(false); };
...
const layoutOrbitPlans = (plans = []) => {
  const lanes = [];
  const arranged = [...plans].sort((a, b) => mins(a.startTime) - mins(b.startTime) || mins(a.endTime) - mins(b.endTime)).map((plan) => {
    const start = mins(plan.startTime);
    const end = mins(plan.endTime);
    let lane = lanes.findIndex((availableAt) => start >= availableAt);
    if (lane === -1) { lane = lanes.length; lanes.push(end); }
    else lanes[lane] = end;
    return { ...plan, orbitLane: lane };
  });
  return { plans: arranged, depth: Math.max(1, lanes.length) };
};
...
function PlanModal({ close, save, existing }) {
  ...
  const submit = async (event) => { event.preventDefault(); if (!form.title.trim()) return setError('Give this plan a clear name.'); if (mins(form.endTime) <= mins(form.startTime)) return setError('End time must be later than start time.'); save({ ...form, title: form.title.trim() }); };
...
function Planner({ username, viewOnly = false, onBack, compareUser = '' }) {
  ...
  const orbitRows = rows.map((row) => ({ ...row, orbit: layoutOrbitPlans(row.plans) }));
...
function InvitePage({ onJoin, ready = true }) {
  const params = new URLSearchParams(window.location.search);
  const from = params.get('from') || 'A Chronos friend';
  const to = params.get('to') || 'you';
  return <main className="invite-page"><section className={`invite-stage cinematic-invite ${ready ? 'invite-ready' : ''}`} aria-label="Chronos invitation"><div className="invite-vault" aria-hidden="true"><i/><i/><i/><i/><i/></div><div className="envelope-wrap"><div className="envelope-shadow"/><div className="envelope"><div className="envelope-back"/><div className="envelope-letter"><span>CHRONOS INVITATION</span><h1>{from} saved a seat for {to}.</h1><p>Step into a calmer way to plan time, meet in private rooms, and see your day with intention.</p></div><div className="envelope-left"/><div className="envelope-right"/><div className="envelope-front"/><div className="envelope-flap"/></div></div><div className="invite-copy"><span>PRIVATE TIME NETWORK</span><h2>Your time. Now in orbit.</h2><p>Create your Chronos account, add your friend, and open a private room when you are ready.</p><button className="gold-button invite-accept" onClick={onJoin}>Accept invitation</button></div></section></main>;
}

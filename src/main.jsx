...
const layoutOrbitPlans = (plans = []) => {
  const lanes = [];
  const sorted = [...plans].sort((a, b) => mins(a.startTime) - mins(b.startTime) || mins(a.endTime) - mins(b.endTime));
  const arranged = sorted.map((plan) => {
    const start = mins(plan.startTime);
    const end = mins(plan.endTime);
    const isOverlap = sorted.some((other) => other.id !== plan.id && start < mins(other.endTime) && end > mins(other.startTime));
    let lane = lanes.findIndex((availableAt) => start >= availableAt);
    if (lane === -1) { lane = lanes.length; lanes.push(end); }
    else lanes[lane] = end;
    return { ...plan, orbitLane: lane, isOverlap };
  });
